  ...
  function CanvasTextWrapper(canvas, text, options) {
    ...
    function render() {
      if (opts.sizeToFill) {
        ...
        var newFontSize = 1;
        var fontSizeHasLimit = opts.maxFontSizeToFill !== undefined;
    ...
    function setVertAlign() {
      ...
      } else if (opts.verticalAlign.match(/px/)){
        let items = opts.verticalAlign.split(/px/)
        textPos.y = (EL_HEIGHT - textBlockHeight) / 2 + Number(items[0]);
      } else {
